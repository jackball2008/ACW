...
{   
   ...
   float specularShininess = 0.5;
   
   vec3 normalVector = normal;
   if (applyParallaxMap && applyTexture)
   {
      // Parallax map
      vec2 parallaxScaleBias = vec2(2.0,-1.0);
      parallaxScaleBias = parallaxScaleBias * 0.02;
      float height = texture2D(heightMapTexture, texture_coordinate).r;
      height = height * parallaxScaleBias.x + parallaxScaleBias.y;
      vec2 offset = (height * eyeVector.xy);
      offset.y *= -1;
      ...
      vec2 parallaxCoords = texture_coordinate + offset;
      textureCoefficient = texture2D(ColorTexture, parallaxCoords);
   }
   if(applyNormalMap)
   {
